...
SELECT * FROM Multiplex mul, Theater t, Movie mov WHERE mul.seq = t.multiplex_seq AND t.movie_seq = mov.seq;

CREATE VIEW information AS
SELECT mul.seq as mul_seq, mul.name as mul_name, mul.address as mul_address, mul.axis as axis, t.seq as theater_seq, t.name as theater_name, t.total_seat as total_seat, t.start_time as start_time, t.end_time, price, movie_seq, title as movie_title, count as movie_count, grade as movie_grade, released, info as movie_info, synopsys, male_p, female_p, trailer_url, trailer_main, pic_main, pic_director, name_director, pic_actor, name_actor
