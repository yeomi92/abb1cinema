...
abb1.controller =(function() {
	                  ...
	                  $.each(data.infoList, function(i, info){
	                      ...
	                      view= '<div id="mypage_table'+i+'" style="margin-top: 5px;margin-bottom: 5px;margin-left: 20px;">   <table>'
	                        +'   <tr>'
	                          ...
	                          +'</table></div>';
	                      if(i==0){
	                         console.log('상세보기 클릭 후 다시 그려질 때 i가 0일 때');
	                         $('#mypage_reservation').html(view);
	                      }else{
	                         console.log('상세보기 클릭 후 다시 그려질 때 i가 0이 아닐 때');
	                         $('#mypage_reservation').append(view);
	                      }
	                    });
	                  customerMypageCss();
	                  //customerReservationDetail();
	                  var i = detailId.split('_')[1]*1;
	                  customerRsDetail(i);
	    		      /*$.ajax({
	    		         ...
	    		         success: function(data){
	    		            ...
	    		            //
	    		             $('#close').on('click',function(){
	    		                ...
	    		                $('#close').html('<a id="detail_icon'+i+'" href="#">상세<img src="'+$.context()+'/resources/img/icon/downarrow.png" width="3%" height="3%" alt="" /></a>');
	    		            	//customerMypage();
	    		                //customerReservationDetail();
	...
	var customerRsDetail = function(){
		$('.abb1_detail_icon').on('click',function(){
			var i=$(this).attr('id').split('_')[1];
			console.log('customerRsDetail i값: '+i);
		$.ajax({
	         url: $.context()+'/getReservationDetail',
	         method: 'POST',
	         data: JSON.stringify({
		 ...
		 $('#confirm').on('click',function(){
			 ...
			 $.ajax({
			   ...
			   data: JSON.stringify({
				   ...
				   trailer_main: $('#trailer_main').is(':checked')
			   }),
			   dataType: 'json',
			   contentType: 'application/json',
			   success: function(data){
				  ...
				  $('#delete').on('click',function(){
					  $.ajax({
						   ...
						   success: function(data){
							   ...
							   alert('시퀀스 변경 성공'+changeSeq)
							   console.log('원래 seq: '+seq+', 바꿀 seq: '+changeSeq);
							   $.ajax({
								   url: $.context()+'/put/admin/main/movie',
								   method: 'POST',
								   data: JSON.stringify({
	    ...
	    $.ajax({
	      ...
	      success : function(data){
			...
			if(data.customer!=null){
				console.log(abb1.cookie.getCookie('id'));
				console.log(data.customer.id);
				console.log(abb1.cookie.getCookie('id')==data.customer.id);
				console.log(abb1.cookie.getCookie('id')===data.customer.id);
			...
			$.each(data.comment_list, function(i,comment){
				...
				$('.abb1_bbs_review_result_reg_date').after('<input id="delete'+comment.seq+'" class="deleteComment" type="button" value="x" style="margin-left: 20px; background: white; color: red; border: 1px solid #c7c5c5;">');
			    }
			});
